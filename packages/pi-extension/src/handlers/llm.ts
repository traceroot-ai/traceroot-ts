...
export function registerLlm(rt: Runtime): void {
  ...
  pi.on("message_end", async (raw) => {
    ...
    if (usage) {
      ...
      if (usage.cacheRead != null) setAttr(entry.span, "gen_ai.usage.cache_read_input_tokens", usage.cacheRead);
      if (usage.cacheWrite != null) setAttr(entry.span, "gen_ai.usage.cache_write_input_tokens", usage.cacheWrite);
      if (usage.totalTokens != null) setAttr(entry.span, "traceroot.pi.total_tokens", usage.totalTokens);
      if (usage.cost?.total != null) setAttr(entry.span, "traceroot.pi.cost_total", usage.cost.total);
    }
    if (message.stopReason != null) setAttr(entry.span, "traceroot.pi.finish_reason", message.stopReason);

    // The assistant message is this LLM span's Output; cache it as the session output.
    const outputText = renderMessageContent((message as { content?: unknown }).content, IO_LIMITS.llmOutput);
    if (outputText) {
  ...
  pi.on("turn_end", async (raw) => {
    ...
    const turnIndex = eventIndex !== null && state.llmSpans.has(eventIndex) ? eventIndex : state.currentLlmTurnIndex;
