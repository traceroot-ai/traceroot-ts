...
describe('main-score closure', () => {
  it('late-bound single metric resolves to the emitted name (never the function name)', async () => {
    ...
    const result = await evaluate({
      name: 'r', dataset: DS, task: echo, scorers: [grade as any], transport: fake,
    });
  ...
  it('explicit multi-metric selection is used everywhere', async () => {
    ...
    const fake = new FakeTransport();
    const result = await evaluate({
      name: 'r', dataset: DS, task: echo, scorers: [accuracy as any, format as any],
      mainScore: 'format', transport: fake,
    });
  ...
  it('missing main_score completes the run terminally, then throws', async () => {
    ...
    await assert.rejects(
      () =>
        evaluate({
          name: 'r', dataset: DS, task: echo, scorers: [acc as any],
          mainScore: 'missing', transport: fake,
        }),
  ...
  it('no successful scores is unscored, not ambiguous', async () => {
    ...
    const fake = new FakeTransport();
    const result = await evaluate({
      name: 'r', dataset: DS, task: echo, scorers: [boom as any], transport: fake,
    });
